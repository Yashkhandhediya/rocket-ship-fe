const RightDrawer = ({ isOpen, heading, onClose, children, drawerWrapperClassNames }) => {
  return (
    <>
      <div
        className={`dark:bg-gray-800 fixed right-0 top-0 z-40 h-screen w-[80%] max-w-[80vw] rounded-lg bg-white px-5 py-4 transition-transform ${
        ...
        <div className="w-full h-full overflow-auto pb-5" style={{scrollbarGutter: "stable"}}>{children}</div>
      </div>
      {isOpen && (
