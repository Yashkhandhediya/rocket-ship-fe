...
>>>>>>> 2640b0720c48a58b1eee3ff9142fc8f6220ecc36
    }
  };
  ...
  const filteredModules = modules.filter((module) => !moduleList.includes(module.value));
  const filteredSettingModules = settings_modules.filter(
    (module) => !settingModuleList.includes(module.value),
  );
  ...
  useEffect(() => {
    if (userData) {
      setUserInfo({ ...userInfo, full_name: userData?.user?.name, email_address: userData?.user?.email });
  ...
  const handleAddUser = () => {
    setIsLoading(true);
    axios
      .post(BACKEND_URL + '/roleuser/add_user', {
        first_name: userInfo?.full_name,
        email_address: userInfo?.email_address,
        company_id: parseInt(localStorage.getItem('company_id')),
        created_by: parseInt(localStorage.getItem('company_id')),
  ...
  const handleUpdate = () => {
    setIsLoading(true);
    axios
      .put(BACKEND_URL + `/roleuser/update_user?role_user_id=${userData?.user?.id}`, {
        first_name: userInfo?.full_name,
        email_address: userInfo?.email_address,
        company_id: parseInt(localStorage.getItem('company_id')),
