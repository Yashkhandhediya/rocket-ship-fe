...
const UserList = () => {
  ...
  const handleUser = () => {
    axios
      .get(BACKEND_URL + `/company/get_company_users/?companyId=${id}`, { headers: headers })
      .then((res) => {
  ...
  const handleKYC = (row) => {
    ...
    axios
      .get(
        BACKEND_URL + `/kyc/?id=${row?.id}&type=user_aadhar`,
        { responseType: 'blob' },
        { headers: headers },
      )
      .then((res) => {
    ...
    axios
      .get(BACKEND_URL + `/kyc/?id=${row?.id}&type=selfie`, { responseType: 'blob' }, { headers: headers })
      .then((res) => {
        console.log('Recharge Responsee', res);
        const imgUrl = URL.createObjectURL(res.data);
        setUserImg(imgUrl);
  ...
  const handleAcceptKYC = () => {
    ...
    // const headers={'Content-Type': 'application/json'};
    axios
      .post(BACKEND_URL + `/kyc/kyc_status/?client_type=user&status=${3}&id=${idUser}`, { headers: headers })
      .then((res) => {
