...
const FreezeModal = ({ setShow, data, setLoading, type }) => {
  ...
  const handleUpload = (name, file) => {
    ...
    apiClient
      .post(
        `${BACKEND_URL}/image/upload_image?product_id=${data.id}`,
        { file: file },
      ...
      .then((response) => {
        setWeightFreezeData({
          ...weightFreezeData,
          images: { ...weightFreezeData.images, [name]: response.data.filepath },
        });
        setLoading(false);
      })
      .catch((error) => {
  ...
  const handleWeightFreezeSubmit = () => {
    if (
      weightFreezeData.product_category === '' ||
      weightFreezeData.width === 0 ||
      weightFreezeData.height === 0 ||
      weightFreezeData.length === 0 ||
      weightFreezeData.dead_weight === 0
    ) {
      return toast('Please fill all the details', { type: 'error' });
    }
    if (
      weightFreezeData.images.img_1 === null ||
      weightFreezeData.images.img_2 === null ||
      weightFreezeData.images.length_img === null ||
      weightFreezeData.images.width_img === null ||
      weightFreezeData.images.height_img === null ||
      weightFreezeData.images.weight_img === null
    ) {
      return toast('Please upload all the images', { type: 'error' });
    }
    const headers = { 'Content-Type': 'application/json' };
    apiClient
      .post(`${BACKEND_URL}/weight_freeze/`, weightFreezeData, { headers })
      .then((response) => {
  ...
  useEffect(() => {
    if (
      ...
      weightFreezeData.width !== 0 &&
      weightFreezeData.height !== 0 &&
      weightFreezeData.weight !== 0
    ) {
      const chargeableWeight = Math.max(
        weightFreezeData.weight,
        (weightFreezeData.length * weightFreezeData.width * weightFreezeData.height) / 5000,
      );
          ...
          <div className="relative flex w-full flex-col rounded-lg border-0 bg-white shadow-lg outline-none focus:outline-none">
            ...
            <div className="border-blueGray-200 flex w-full items-center justify-between rounded-t border-b border-solid p-5">
              ...
              <h3 className="text-2xl font-semibold">
                {type == 'Freeze' ? 'Add Product and Package Details' : 'Edit Product and Package Details'}
              </h3>
              <button
            ...
            <div className="relative flex-auto p-6">
              ...
              <div className="m-1 flex flex-col rounded-md border border-gray-200">
                {/* Product Information */}
                <div className="flex h-52 flex-row gap-8 p-4 px-8">
                  ...
                  <div className="flex w-[55%] flex-col gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        id="name"
                        value={data.name}
                        className="mt-2 h-8 w-full rounded-lg border-gray-300 bg-[#ECF1F2] text-[12px] font-normal text-black focus:border-gray-300 focus:ring-0"
                        readOnly
                      />
                    </div>
                    {/* Product Category */}
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setWeightFreezeData({ ...weightFreezeData, product_category: e.target.value })
                        }
                      />
                  ...
                  <div className="flex w-[45%] flex-col">
                    <p className="flex flex-row items-center">
                      ...
                      <CustomTooltip
                        style="dark"
                        text={
                          'Take different pictures of your product with a ruler to show the correct length, width and height Place your product on a weighing scale to show its actual weight.'
                        }
                        placement="right">
                    ...
                    <div className="mt-2 flex flex-row gap-8 rounded-lg">
                      {/* Image 1 */}
                      <div className="flex h-32 w-[40%] flex-col">
                        ...
                        <div className="flex h-32 cursor-pointer flex-col items-center justify-evenly rounded-lg border-2 border-dashed border-[#B02828]">
                          <label htmlFor="img_2" className="w-full">
                            <div className="flex cursor-pointer flex-col items-center justify-center">
                              {images.img_2 ? (
                                <div className="flex h-[90%] w-[90%] justify-center">
                                  <img src={images.img_2} alt="" className="h-28 object-fill" />
                                </div>
                              ) : (
                                <>
                                  <img src={upload} alt="" />
                      ...
                      <div key={key} className="flex flex-col">
                        ...
                        <div>
                          <input
                            ...
                            name={item.name}
                            id={item.id}
                            className={`mt-2 h-8 w-3/4 rounded-l-lg border border-gray-300 text-[12px] font-normal [appearance:textfield] focus:border-gray-300 focus:ring-0 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none ${
                              item.name === 'chargableWeight' && 'bg-[#ECF1F2]'
                            }`}
                            onChange={(e) =>
                              setWeightFreezeData({ ...weightFreezeData, [item.name]: e.target.value })
                            }
                          ...
                          />
                          <button
                            className="h-8 w-1/4 rounded-r-lg bg-[#B07828] text-white focus:outline-none focus:ring-0"
                            disabled>
                            {item.unit}
                          </button>
                        </div>
              ...
              <p className="flex flex-row gap-4 text-lg font-semibold">
                Package Images
                <CustomTooltip
                  style="light"
                  text={<img src={freezeGuide} />}
                  placement="right"
                  wrapperClassNames={'shadow-none p-0 max-w-[400px]'}>
                  <span className="text-[12px] underline">Know more about measuring packages</span>
                </CustomTooltip>
              </p>
              <div className="m-1 flex flex-col rounded-md border border-gray-200">
                {/* Package Images */}
                <div className="flex h-44 flex-row gap-8 p-4 px-8">
                  {packageImagesData.map((item, key) => {
                    return (
                      <div className="flex h-40 w-[40%] flex-col" key={key}>
                        <div className="flex h-32 cursor-pointer flex-col items-center justify-evenly rounded-lg border-2 border-dashed border-[#B02828]">
                          <label htmlFor={item.id} className="w-full">
                            <div className="flex cursor-pointer flex-col items-center justify-center">
                              {item.value ? (
                                <div className="flex h-[90%] w-[90%] justify-center">
                                  <img src={item.value} alt="" className="h-28 object-fill" />
                                </div>
                              ) : (
                                <>
                                  <img src={upload} alt="" />
                        ...
                        {item.value ? (
                          ...
                          <p className="mt-2 py-1 text-center">Package {item.label}</p>
                        )}
                      </div>
                    );
                  })}
                ...
                <div className="flex w-full items-center justify-center border-0 border-t-2 bg-[#f8f8f892] py-2 text-[12px]">
                  Note :&nbsp;
                  <span className="font-normal">Uploaded Images should be less than 5 MB</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center rounded-b-md">
              ...
              <div className="w-full rounded-b-md bg-[#DAE5FF] py-3 text-center font-[400] text-[#525252]">
                Our team will validate the details provided and confirm the status of weight freeze request
                accordingly.
              </div>
            </div>
          </div>
