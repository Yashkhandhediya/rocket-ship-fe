  ...
  const handleCaptureImage = () => {
    ...
    try {
      if (videoRef.current && canvasRef.current) {
        ...
        // apiClient.post(BACKEND_URL + `/kyc/upload_selfie?image_id=${id_user}&type="selfie"&username=${user_name}`,{
        //     file:imageDataURL
        // },{headers}).then((res) => {
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex h-2/3 w-[85%] flex-col items-center justify-center rounded-md bg-white py-12 shadow-md">
        <div
          className={`video_div flex h-full w-1/2 items-center justify-center rounded-2xl border-2 border-dashed border-red-200 bg-[#FAFAFA] ${
        ...
        <div>
          <button
            type="button"
            className="dark:focus:ring-red-900 rounded-lg border-2 border-red-500 bg-white px-14 py-1.5 text-sm font-medium text-red-500 hover:bg-red-500 hover:text-white focus:outline-none focus:ring-4 focus:ring-red-300"
