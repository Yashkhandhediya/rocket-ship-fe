...
const Adhaar_Document = ({ triggerValidation, setTriggerValidations, setIsKYCCompleted }) => {
  ...
  const handleSendOTP = () => {
    try {
      ...
      apiClient.post(BACKEND_URL + `/kyc/adhaar_generate_otp?id_number=${temp_number}`).then((res) => {
    ...
    <div>
      ...
      {!showOTPBox && (
        <div className="mt-6 flex justify-start gap-4">
          <button
            type="button"
            className={`dark:focus:ring-red-900 rounded-lg ${
              isValidAdhaar && adhaarNumber.length
                ? 'bg-red-600'
                : 'border border-[#e5e5e5] bg-red-600 text-[#c6c6c6]'
            } px-8 py-2 text-[0.8rem] font-medium text-white transition-colors duration-300 focus:outline-none focus:ring-4 focus:ring-red-300`}
