...
const OTP_Input = ({ handleSendOTP, timer, setIsKYCCompleted, id = null }) => {
  ...
  const completeKYC = () => {
    if (isOtpEntered) {
      ...
      apiClient
        .post(BACKEND_URL + `/kyc/adhaar_submit_otp?reference_id=${id}&otp=${temp_otp}`)
        .then((res) => {
      ...
      <div className="mt-2 flex items-center justify-start gap-4 text-[12px] font-medium">
        <button
          className={`px-12 py-2 text-white ${
            isOtpEntered ? 'bg-red-600' : 'border border-[#e5e5e5] bg-red-600 text-[#c6c6c6]'
        ...
        {seconds > 0 ? (
          ...
          <button
            className="cursor-pointer text-[#7973ef]"
            onClick={() => {
              setSeconds(timer);
              handleSendOTP();
            }}>
