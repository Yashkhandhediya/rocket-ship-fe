...
const Company_profile = () => {
  ...
  const [companyDetails, setCompanyDetails] = useState({
    companyId:
      localStorage.getItem('company_id') != 'undefined' ? localStorage.getItem('company_id') : '12345678',
    companyName: '',
    website: '',
    email: '',
    logo: '',
  ...
  const handleData = () => {
    ...
    apiClient
      .get(BACKEND_URL + `/company/${localStorage.getItem('company_id')}`)
      .then((res) => {
          ...
          <div className="flex min-h-72 w-full flex-col gap-5 border px-3 py-5 text-[12px] font-bold text-[#666666]">
            ...
            <div className="flex w-1/2 flex-col items-start justify-start gap-2">
              <label htmlFor="logo" className="text-[12px] font-semibold">
                Website/Company Logo <span className="text-[16px] font-bold text-red-500">*</span>
              </label>
              <div className="relative">
                {companyDetails.logo ? (
                  <div className="relative flex h-28 w-28 items-center justify-between bg-gray-200">
                    <span></span>
                    <img src={companyDetails.logo} alt="logo" className="h-[6.5rem] w-20" />
                    <span
                      className="m-0 h-full p-0 text-right font-bold"
                  ...
                  <>
                    ...
                    />
                    <label htmlFor="logo" className="cursor-pointer rounded border p-2 shadow">
