...
const DetailsCard = ({ children, headingIcon, heading, headingClassNames, toggle }) => {
  ...
  return (
    <div className="w-full">
      <div className="my-4  max-w-full overflow-auto rounded-lg bg-white p-4 pt-2.5 shadow-[0_1px_2px_rgba(0,0,0,0.161)]">
        <div
        ...
        <div className={`${toggleBody ? 'h-auto' : 'h-0 overflow-hidden'}`}>{children}</div>
      </div>
    </div>
  );
};
