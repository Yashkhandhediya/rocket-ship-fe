...
const Tabs = ({ tabs, tabClassNames, activeReturnTab, setReturnActiveTab, onTabChange = () => {} }) => {
  ...
  const tabID = tabs.find((_, i) => i === activeReturnTab);
  ...
  const handleClick = (index) => {
    setReturnActiveTab(index);
    onTabChange(index);
    localStorage.setItem('activeReturnTab', JSON.stringify(index));
  };
            ...
            <li className={`me-2 flex`} key={tab.id}>
              <button
                ...
                className={`me-2 inline-flex items-center rounded-t-lg border-b-4 p-2 ${
                  i === activeReturnTab ? 'border-red-600 text-rose-500' : 'border-transparent text-[#9CA3AF]'
                } ${tabClassNames}`}
                id={`${tab.id}-tab`}
                type="button"
